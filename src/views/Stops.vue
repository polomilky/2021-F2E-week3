<template>
    <v-container class="stops">
        <Header />
        <Section @listenToChildStopEvent="getDatafromChild" />
        <StopsBusArea
            :getGoBusData = "goBusData"
            :getBackBusData = "backBusData"
            v-if="this.goBusData.length !== 0 || this.backBusData.length !== 0"
        />
        <NoResult
            v-show="show.noResult"
        />
    </v-container>
</template>
...
<script>
    ...
    import StopsBusArea from '../components/StopsBusArea'
    ...
    export default {
        ...
        methods: {
            getDatafromChild(itemGo, itemBack) {
                this.goBusData = itemGo;
                this.backBusData = itemBack;
                if (itemGo.length === 0 && itemBack.length === 0) {
                    this.show.noResult = true;
                } else {
                    this.show.noResult = false;
                }
            }
        }
    }
...
<style lang="sass" scoped>
    ...
    .road
        background: $unnamed-color-f9fcf4
</style>
